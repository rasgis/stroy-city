...
class ImageService {
  ...
  async getImages(type?: ImageType): Promise<ImageInfo[]> {
    try {
      const url = `${API_CONFIG.BASE_URL}/api/images${type ? `?type=${type}` : ''}`;
   ...
   * @param type Тип изображения (product/category/other)
   * @returns URL загруженного изображения
   */
  async uploadImage(file: File, type: ImageType = 'other'): Promise<string> {
    try {
      // Используем fileService для загрузки файла
      const imageUrl = await fileService.saveImage(file, type === 'product' || type === 'category' ? type : undefined);
      return imageUrl;
    } catch (error) {
